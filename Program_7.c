...
typedef struct {
    ...
    bool visited[MAX_VERTICES];   // If vertex has been processed
} PathInfo;
...
void addEdge(Graph *g, int src, int dest, int weight) {
    ...
    // Adds edge from destination to source (undirected graph)
...
int findMinDistance(PathInfo *info, int vertices) {
    ...
    for (v = 0; v < vertices; v++) {
        // If vertex isnt visited and has smaller distance
